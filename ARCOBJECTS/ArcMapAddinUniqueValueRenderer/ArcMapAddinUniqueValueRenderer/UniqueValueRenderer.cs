...
{
    public class UniqueValueRenderer : ESRI.ArcGIS.Desktop.AddIns.Button
    {
        private static readonly string SRC = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "data");
        private static readonly string LPK = Path.Combine(SRC, "usstpln83ca_relateAndJoin.lpk");

        private const string TableName = "zoneTable";
        private const string JoinField = "FKEY";
        ...
        private static IUniqueValueRenderer GenerateUniqueValueRenderer(IFeatureLayer featureLayer)
        {
            ...
            while (pEnumerator.MoveNext())
            {
                ...
                SimpleFillSymbolClass fillSymbol = new SimpleFillSymbolClass { Color = fillColor };
